...
# GCC_ARM_NONE_EABI_PATH = ~/dev/gcc-arm-none-eabi-5_4-2016q3/bin
GCC_ARM_NONE_EABI_HOME = /mnt/e/dev/gcc-arm-none-eabi-5_4-2016q3-20160926-win32
GCC_ARM_NONE_EABI_PATH =  $(GCC_ARM_NONE_EABI_HOME)/bin
ARM_NONE_EABI_PATH =  $(GCC_ARM_NONE_EABI_HOME)/arm-none-eabi/bin

CC = $(GCC_ARM_NONE_EABI_PATH)/arm-none-eabi-gcc.exe
AR = $(GCC_ARM_NONE_EABI_PATH)/arm-none-eabi-ar.exe
OBJCOPY = $(GCC_ARM_NONE_EABI_PATH)/arm-none-eabi-objcopy.exe
OBJDUMP = $(GCC_ARM_NONE_EABI_PATH)/arm-none-eabi-objdump.exe
SIZE = $(GCC_ARM_NONE_EABI_PATH)/arm-none-eabi-size.exe
...
OPENOCD_PROC_FILE=stm32f7discovery.cfg
...
CFLAGS = -Wall -g -Os -D $(CPU_MODEL_GENERAL) -include stm32f7xx_hal_conf.h -Werror-implicit-function-declaration
...
stlink:
	$(STLINK_CLI) -c SWD -p  "`pwd`/Build/$(PROJ_NAME).hex" -Rst -Run
